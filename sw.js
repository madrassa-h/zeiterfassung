...
const CACHE = 'madrassa-v4'; // ← Bei jedem Deploy um 1 erhöhen → v5, v6 …
...
self.addEventListener('install', function(e) {
  ...
  self.skipWaiting(); // Sofort aktiv, nicht auf Tab-Schließen warten
});
...
self.addEventListener('fetch', function(e) {
  // Firebase, Firestore, EmailJS, CDNs → immer Netzwerk, nie cachen
  ...
  if (url.includes('.html') || url === location.origin + '/' || url.endsWith('/')) {
    e.respondWith(
      fetch(e.request)
        .then(function(res) {
          var clone = res.clone();
