/* Madrassa Hannover – Service Worker v2 */
...
self.addEventListener('notificationclick', function(e) {
  ...
  var target = (e.notification.data && e.notification.data.url) || './modir.html';
  e.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then(function(list) {
      for (var i = 0; i < list.length; i++) {
        if (list[i].url.includes(target) && 'focus' in list[i]) {
          return list[i].focus();
        }
      }
